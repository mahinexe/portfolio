...
const setCounterText = (target: HTMLElement | null, value: number | null): void => {
  if (!target) {
    return;
  }
...
const readCounterValue = async (path: string): Promise<number | null> => {
  try {
...
const updateTotalViews = async (): Promise<void> => {
  if (!totalViewsTarget) {
    return;
  }
...
const refreshLiveUsers = async (): Promise<void> => {
  if (!liveViewsTarget) {
    return;
  }
...
const registerLiveUser = async (): Promise<void> => {
